...
**AI / Ollama:** all model calls go through `services/ollama.py` — never raw `httpx`. Use `call_text_json(prompt, system=…, timeout=…)` (sends `format:"json"`, strips fences, retries once, raises `OllamaError`). It's local-only and env-driven (`OLLAMA_BASE_URL` defaults to `host.docker.internal:11434` in compose, `OLLAMA_TEXT_MODEL`). There's also an admin "LLM provider" switch (`ai_service`) that can route to Anthropic — call `call_text_json` directly when a feature must stay Ollama-only. **Never trust model output:** validate/normalise server-side and never let a model call 500 a user flow. Example: plant prefill (`POST /api/plants/prefill`, `services/plant_prefill_service.py`) maps a name → advisory create-form values with a location synonym map + int clamping + dropped-unknown-keys, a 45s timeout, and an `ok=false` "manuell ausfüllen" fallback on any failure. Edibility is advisory-only (`edible_suggestion`/`edible_note`) and the real `edible` field is never auto-set — the frontend prefill is non-blocking and only fills fields still at their default/empty value.
