...
**Plants also have month/season care (calendar-based, distinct from the day intervals):** `fertilize_start_month`/`fertilize_end_month` gate the fertilize reminder to a season (`fertilize_in_season()`, wrap-around supported; applied in `fetch_due_care`, `due_this_week`, and the serialized `fertilize_due` so out-of-season pauses everywhere). `prune_month` drives an **annual** reminder (`fetch_due_prune` + `notify_plant_care(kind="prune")`); `prune_reminder_year` is the per-year dedup, reset when `prune_month` changes. `bloom_start_month`/`bloom_end_month` are display-only. All are months 1–12 (nullable), filled by the AI prefill, and surfaced in the form (month dropdowns) + the detail "Pflege auf einen Blick" grid.
