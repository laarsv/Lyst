  ...
  return (
    <div className="max-w-xl mx-auto p-4 sm:p-6">
      ...
      <div
        ...
        style={{ borderTopColor: data.color || '#00c896', borderTopWidth: 4 }}
      >
        ...
        {data.description && <p className="text-sm text-muted mt-3">{data.description}</p>}
      </div>
          ...
          {data.items.map((it) => (
            <li key={it.id} className="py-2.5 px-2 flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className={it.is_checked ? 'line-through text-muted/70' : ''}>{it.text}</div>
      ...
      <p className="text-center text-xs text-muted/70 mt-6">Schreibgeschützte Ansicht</p>
    </div>
  );
}
